...
const Profile = () => {
  ...
  const studentData = useAppSelector<ICurrentUser | null>(
  ...
  return (
    <div className="profile-container">
      <div className="flex flex-col mx-auto md:mx-0 text-center justify-center p-5 border-b-2 border-b-gray-400 md:border-b-0 md:border-r md:border-r-gray-400">
        <div className="w-36 mb-5 rounded-full border-1 mx-auto">
          <label htmlFor="uploadImage" className="relative">
            <img
              src={studentData?.currentUser?.profilePic}
              alt="Student avatar"
              className="rounded-full w-36 h-36 p-1 "
            />
            <span className="text-xs absolute bottom-2 right-2 rounded shadow-sm text-white bg-black bg-opacity-70 p-1">
              Change
            </span>
          </label>
          <input
        ...
        <div className="hidden md:block">
          <h1 className="normal-case">{studentData?.currentUser?.name}</h1>
        </div>
        <h1 className="md:hidden normal-case">
          {studentData?.currentUser?.name}
        </h1>
        <h3>{studentData?.role}</h3>
      </div>
      {/* right side */}
      <div className="flex flex-col my-4 px-5 grow">
        <div className="flex flex-col md:flex-row justify-between gap-2 pb-3 border-b border-b-gray-400">
          <div>
            <h4>Mail</h4>
            <p className="text-sm">{studentData?.currentUser?.email}</p>
          </div>
          <div>
            <h4>Mobile</h4>
            <p className="text-sm">{studentData?.currentUser?.mobile}</p>
          </div>
        </div>
        <div className="flex flex-col items-center">
          ...
          {message && (
            <span className="text-center text-md text-red-700 font-semibold">
              {message}
            </span>
          )}
        </div>
